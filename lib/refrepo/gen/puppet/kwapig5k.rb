...
def generate_puppet_kwapig5k(options)

  puts "Writing kwapi configuration files to: #{options[:output_dir]}"
  ...
  refapi['sites'].each { |site_uid, site|
    ...
    site['clusters'].sort.each { |cluster_uid, cluster|
      cluster['nodes'].each_sort_by_node_uid { |node_uid, node|
        ...
        output = ERB.new(File.read(File.expand_path('templates/kwapi-node.erb', File.dirname(__FILE__))), nil, '-').result(binding)
        output_file = Pathname("#{options[:output_dir]}//platforms/production/modules/generated/files/grid5000/kwapi/#{site_uid}/#{node_uid}.conf")
    ...
    site['network_equipments'].each { |netdev_uid, netdev|
      ...
      output = ERB.new(File.read(File.expand_path('templates/kwapi-netdevice.erb', File.dirname(__FILE__))), nil, '-').result(binding)
      output_file = Pathname("#{options[:output_dir]}//platforms/production/modules/generated/files/grid5000/kwapi/#{site_uid}/#{netdev_uid}.conf")
      output_file.dirname.mkpath()
      File.write(output_file, output)
    }
    ...
    if not wattmetre_port_per_node.empty?
      output = ERB.new(File.read(File.expand_path('templates/kwapi-wattmetre-mapping.erb', File.dirname(__FILE__))), nil, '-').result(binding)
      output_file = Pathname("#{options[:output_dir]}//platforms/production/modules/generated/files/grid5000/kwapi-wattmetre/#{site_uid}/wattmetre-mapping.conf")
      output_file.dirname.mkpath()
      File.write(output_file, output)
    end
  }
